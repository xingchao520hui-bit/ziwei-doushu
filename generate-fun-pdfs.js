...
async function generatePDFs() {
  ...
  for (let i = 0; i < people.length; i++) {
    ...
    const url = `${SERVER_URL}/ziwei-fun.html?year=${year}&month=${month}&day=${day}&hour=${p.hour}&gender=${p.gender}`;
    ...
    try {
      ...
      // 等待页面加载完成（查找结果区域）
      await page.waitForSelector('.result-area, .content', { timeout: 20000 }).catch(() => {});
      // 额外等待，让JavaScript执行完
